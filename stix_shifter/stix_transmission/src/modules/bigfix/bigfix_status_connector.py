...
class BigFixStatusConnector(BaseStatusConnector):
    ...
    @staticmethod
    def get_succes_status(data_dict):
        ...
        try:
            ...
            if 199 < response_code < 300:
                try:
                    ...
                    if return_obj['success'] and return_obj['status'] == Status.COMPLETED.value:
                        if not self.get_succes_status(response_dict):
                            return_obj['status'] = Status.ERROR.value
                            ErrorResponder.fill_error(return_obj, response_dict, ['results', '+isFailure=True',
                                                                                  '~result'])
