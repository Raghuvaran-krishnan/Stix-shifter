...
class RelevanceQueryStringPatternTranslator:
    ...
    stix_object_format_string_lookup_dict = {'file': '''("{}", name of it | "n/a",
                    "sha256", sha256 of it | "n/a",
                    "sha1", sha1 of it | "n/a",
                    "md5", md5 of it | "n/a",
                    pathname of it | "n/a",
                    (modification time of it - "01 Jan 1970 00:00:00 +0000" as time)/second )
                    of {} whose (name of it as lowercase = {} as lowercase)'''}
    # query_format = {
    ...
    def _parse_expression(self, expression, qualifier=None):
        if isinstance(expression, ComparisonExpression):  # Base Case
            ...
            comparison_string = self._parse_mapped_fields(expression, value, comparator, stix_object,
                                                          mapped_fields_array,
                                                          self.stix_object_format_string_lookup_dict.get(stix_object))
    ...
    @staticmethod
    def _parse_mapped_fields(expression, value, comparator, stix_object, mapped_fields_array, stix_format_string):
        comparison_string = ""
        master_stix_object_mapping = {'file': 'files',
                                      'process': 'processes'}
        ...
        for mapped_field in mapped_fields_array:
            if expression.comparator == ComparisonComparators.NotEqual or \
                ...
                comparison_string += stix_format_string.format(stix_object, master_stix_object_mapping.get(stix_object),

                                                               value) if stix_format_string else ""
...
def translate_pattern(pattern: Pattern, data_model_mapper, options):
    ...
    final_query = translated_dictionary.queries
